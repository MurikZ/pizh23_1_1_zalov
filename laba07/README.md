...
### 3. Временная сложность построения кучи из массива
- **Сложность:** `O(n)`.  
- **Почему не O(n log n)?**
  - Метод «просеивания вниз» (sift_down) начиная с последних внутренних узлов до корня:
    - Узлы на глубине `h` выполняют `O(h)` операций.
    - Количество узлов на глубине `h` примерно `n / 2^(h+1)`.
    

---

### 4. Алгоритм пирамидальной сортировки (Heapsort)
1. Построить max-кучу из массива.
2. Повторять до пустоты:
   - Меняем местами корень (максимум) с последним элементом массива.
   - Уменьшаем размер кучи на 1.
   - Выполняем `sift_down` для нового корня, чтобы восстановить свойство max-кучи.
3. В результате элементы массива будут отсортированы по возрастанию.  

**Сложность:** `O(n log n)` (n операций извлечения × log n на восстановление кучи).

---

### 5. Использование кучи для приоритетной очереди
- Куча позволяет эффективно поддерживать **приоритетную очередь**, так как:  
  - Извлечение элемента с максимальным/минимальным приоритетом: `O(log n)`  
  - Вставка нового элемента: `O(log n)`  
  - Замена ключа (увеличение/уменьшение приоритета): `O(log n)`  
- Это делает кучу оптимальной для реализации очередей, где часто требуется быстро получать элемент с наивысшим или наименьшим приоритетом.
